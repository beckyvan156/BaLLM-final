...
output_path = r"C:\Users\becky\OneDrive\Desktop\2023 summer intern\literature\CARD\results\text_final_kg.csv"
...
output_path = r"C:\Users\becky\OneDrive\Desktop\2023 summer intern\literature\CARD\results\final_kg.csv"
...
# Compute group means
group_means = predicted_pheno.groupby('phenotype')['Score'].mean()

plt.figure(figsize=(8, 6))
sns.boxplot(data=predicted_pheno, x='phenotype', y='Score', palette='Set2')

# Add mean values below the boxes
y_min = predicted_pheno['Score'].min()
offset = (predicted_pheno['Score'].max() - y_min) * 0.1  # control spacing

for i, (phenotype, mean_val) in enumerate(group_means.items()):
    plt.text(i, y_min - offset, f'mean = {mean_val:.2f}', ha='center', va='top', fontsize=10, fontweight='bold')

plt.title('Distribution of Score by Phenotype')
plt.xlabel('Phenotype')
plt.ylabel('Score')
plt.ylim(y_min - 2 * offset, 3)  # extend y-axis to make space
plt.tight_layout()
plt.show()
